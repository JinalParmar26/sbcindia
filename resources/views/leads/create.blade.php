        ...
        <form action="{{ route('leads.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="row">
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select name="lead_owner_id" class="form-select">
                        ...
                        @foreach($users as $user)
                            <option value="{{ $user->id }}" {{ old('lead_owner_id') == $user->id ? 'selected' : '' }}>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" name="industry" class="form-control" value="{{ old('industry') }}" placeholder="e.g., Technology, Healthcare">
                    @error('industry') <small class="text-danger">{{ $message }}</small> @enderror
                </div>

                <div class="col-md-6 mb-3">
                    <label class="form-label">Lead Source</label>
                    <input type="text" name="lead_source" class="form-control" value="{{ old('lead_source') }}" placeholder="e.g., Website, Referral, Cold Call">
                    @error('lead_source') <small class="text-danger">{{ $message }}</small> @enderror
                </div>

                <div class="col-md-6 mb-3">
                    <label class="form-label">Price Group</label>
                    <input type="text" name="price_group" class="form-control" value="{{ old('price_group') }}" placeholder="e.g., Premium, Standard, Basic">
                ...
                <div class="col-md-4 mb-3">
                    ...
                    <select name="visit_status" class="form-select">
                        @foreach($visitStatuses as $visitStatus)
                            <option value="{{ $visitStatus }}" {{ old('visit_status', 'Not Started') == $visitStatus ? 'selected' : '' }}>
                                {{ $visitStatus }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-4 mb-3">
                    ...
                    <input type="datetime-local" name="visit_started_at" class="form-control" value="{{ old('visit_started_at') }}">
                    @error('visit_started_at') <small class="text-danger">{{ $message }}</small> @enderror
                </div>

                <div class="col-md-4 mb-3">
                    <label class="form-label">Visit Ended At</label>
                    <input type="datetime-local" name="visit_ended_at" class="form-control" value="{{ old('visit_ended_at') }}">
                ...
                <div class="col-md-3 mb-3">
                    ...
                    <select name="deal_status" class="form-select">
                        ...
                        @foreach($dealStatuses as $dealStatus)
                            <option value="{{ $dealStatus }}" {{ old('deal_status') == $dealStatus ? 'selected' : '' }}>
                                {{ $dealStatus }}
                            </option>
                        @endforeach
                    </select>
